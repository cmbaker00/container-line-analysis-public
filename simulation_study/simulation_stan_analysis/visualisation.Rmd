...
raw_data %>%
  mutate(
    ratio = abs(summary_mean_all/summary_mean_line)
  ) %>%
...
```{r raw-line-cont-old, fig.cap="Comparing estimates of p_intercept[i] for all data and line only data. Using original prior."}
old_raw_data %>%
  filter(str_detect(param_name, "p_intercept")) %>%
  ggplot(aes(x = summary_mean_all, y = summary_mean_line, color = param_name)) +
  geom_point() +
