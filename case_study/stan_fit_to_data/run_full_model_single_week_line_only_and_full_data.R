...
get_data_filename <- function (week, path, extension){
  data_filename <- paste0('furniture_data_2020_week_', week, '_inspected')
  if (path==TRUE){
    prefix <- 'case_study/data_for_stan/'
  } else {
    prefix <- ''
  }
  ...
  return(paste0(prefix, data_filename, suffix))
}

read_data <- function (week){
  read.csv(get_data_filename(week, path=TRUE, extension=TRUE)) %>% mutate(Documentation = as.numeric(Documentation))
}
...
run_stanfit_single_week <- function (week){
  ...
  data_filename <- get_data_filename(week, FALSE, FALSE)
