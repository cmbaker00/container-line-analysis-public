...
\author{Christopher M.\ Baker\(^{1,2,3}\) \and Howard Bondell\(^{1,2}\) \and Nathaniel Bloomfield\(^{1,2,3}\) \and Elena Tartaglia\(^{4}\) \and Thomas K.\ Waring\(^{1,2,3}\)   \and Andrew P.\ Robinson\(^{1,3}\)}
...
\begin{enumerate}
\item School of Mathematics and Statistics, The University of Melbourne
\item Melbourne Centre for Data Science, The University of Melbourne
\item Centre of Excellence for Biosecurity Risk Analysis, The University of Melbourne
\item Data61, Commonwealth Scientific and Industrial Research Organisation
\end{enumerate}

\noindent
cbaker1\at unimelb.edu.au\\
howard.bondell\at unimelb.edu.au\\
nathaniel.bloomfield\at unimelb.edu.au  \\
elena.tartaglia\at data61.csiro.au \\
tom.waring\at unimelb.edu.au \\
...
\noindent%
{\it Keywords:} Bayesian inference;  risk analysis; cargo 
...
Figure~\ref{fig:simulation_estimation} shows the that the STAN model gives good parameter estimates, if there is sufficient data. \hl{Overall, the STAN estimates are close to the true values when there is a large amount of data}. The two smallest values of \(\alpha\) show the worst performance, which is not surprising, given our asymptotic analysis results (Section~\ref{sec:asymptotic_analysis}).  
...
\hl{\textbf{NEW FIGURE DISCUSSION}}
\begin{itemize}
  \item Figure~\ref{fig:SD_est_simulated} to be removed: some counter-intuitive results probably due to stochastic variation (in the container generation) rather than actual results. See eg entry size 20 and $alpha=-3.9$, estimates get for more lines and $100\%$ container mode.
  \item \hl{New version} of Figure~\ref{fig:simulation_estimation}. Previously, the model was fit to some number of lines, all using one batch of data (list of containers), the spread was the standard error in the MCMC samples. Complaint with this was it didn't actually show the model produces reliable estimates, as it was only tested once.
