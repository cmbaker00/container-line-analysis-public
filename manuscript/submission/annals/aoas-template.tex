...
j &= 1, 2,\text{\hspace{70pt}without and with documentation}\\
k &= 1,\ldots, d, \quad d \in \mathbb{N},\quad d = \text{\# countries}\\
\ell &= 1,\ldots,g, \quad g\in \mathbb{N},\quad g = \text{\# entries}.
\end{split}
\end{align}
...
\begin{align}
\begin{split}
\mathbb{I}_1 & = 0,\quad\text{without documentation}\\
\mathbb{I}_2 &= 1,\quad\text{with documentation}.
\end{split}
\end{align}
...
Analysing the full data set, rather than the line-only data, gives us information about more items and countries. Table~\ref{tab:estimates_only_from_all_data} shows all of the parameter estimates that we only get when including the container mode data in the analysis. In this data-set, there are 11 parameters that can only be estimated using the full data-set, because there is no data in the line-only data that correspond to these parameters.
