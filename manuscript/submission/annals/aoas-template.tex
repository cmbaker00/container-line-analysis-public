...
\runtitle{GLM for partially pooled categorical predictors}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\author[A,B]{\fnms{Christopher M.}~\snm{Baker}\ead[label=e1]{cbaker1@unimelb.edu.au}},
\author[B]{\fnms{Howard}~\snm{Bondell}\ead[label=e2]{howard.bondell@unimelb.edu.au}},
\author[A,B]{\fnms{Evelyn}~\snm{Mannix}\ead[label=e3]{evelyn.mannix@unimelb.edu.au}},
\author[C]{\fnms{Elena}~\snm{Tartaglia}\ead[label=e4]{elena.tartaglia@data61.csiro.au}},
\author[B]{\fnms{Thomas K.}~\snm{Waring}\ead[label=e6]{tom.waring@unimelb.edu.au}}
\and
\author[A]{\fnms{Andrew P.}~\snm{Robinson}\ead[label=e5]{apro@unimelb.edu.au}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Addresses                                %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\address[A]{Centre of Excellence for Biosecurity Risk Analysis, The University of Melbourne\printead[presep={,\ }]{e1,e3,e5}}
\address[B]{Melbourne Centre for Data Science, The University of Melbourne\printead[presep={,\ }]{e2,e6}}
\address[C]{Data61, Commonwealth Scientific and Industrial Research Organisation\printead[presep={,\ }]{e4}}
\end{aug}
...
Invasive species pose a multifaceted threat to society, leading to reductions in agricultural productivity, as well as damages to the environment, human health, and the economy \cite{kumar_rai_invasive_2020}. Considerable effort is devoted to managing invasive species \cite{jardine_estimating_2018}, in either eradicating them \cite{baker_recent_2020,holmes_globally_2019, wenger_estimating_2017,helmstedt_prioritizing_2016}, or suppressing their numbers to reduce damages \cite{binny_long-term_2021,brook_effects_2012,sharov_slow_2002}. The costs associated with managing invasive species provides governments with an incentive to manage biosecurity risks at national borders to prevent the establishment of the new species. 
...
\begin{align}
\mathbb{P}\left(\text{Entry } k \text{ non-compliant}\right) = q_k = 1-\prod_{\text{lines } n \text{ in entry } k}[1-p(n)],\label{eq:entry_pr}
\end{align}
where  $p(n)$ is calculated from Eq.~\eqref{eq:logit_model_sim}.
...
% \textcolor{blue}{[ET: What is $N_{E,S}$?]} 
...
% Eq.~\eqref{eq:loglikelihood_arbitrary_entry_size} \textcolor{blue}{[ET: Missing equation reference.]}, we get
...
\begin{align}
  \logit(p(n)) = \alpha_{i(n)} + \beta\mathbb{I}_\text{doc}(n) + \delta_{j(n)} + \gamma_{k(n)}
\end{align}
where 
...
\delta_j &\sim \Normal(0, 0.5),\quad \quad  j = 1,\ldots, d, \quad \quad d \in \mathbb{N}\\
\sigma &\sim \Normal(0, 0.5)
\end{split}
\end{align}

As above, $p(n)$ is the probability that line $n$ is non-compliant. %We denote the probability that line $n$ is non-compliant as $p(n)$, where $p(n) = p_{ijk}$ if that line has characteristics $(i,j,k)$.
...
%\begin{align}
%\text{Non-compliant}_{n, \text{line}} \sim \text{Bernoulli}(p(n)).
%\end{align}
%
...
%\begin{align}
%\mathbb P(\text{Entry } \ell \text{ non-compliant}) = q_\ell = 1 - \prod_{n\in\text{lines}(\ell)} (1-p(n)),
%\end{align}
...
% \textcolor{blue}{[ET: Refer to Fig 1 where SE decreases with number of lines? Maybe instead of ``smaller values" say explicitly ``fewer lines"]}).  
...
%
%
%{\color{red}\textbf{NEW FIGURE DISCUSSION}}
%
%
%\begin{itemize}
%  \item {\color{red}New version} of Figure~\ref{fig:simulation_estimation}. Previously, the model was fit to some number of lines, all using one batch of data (list of containers), the spread was the standard error in the MCMC samples. Complaint with this was it didn't actually show the model produces reliable estimates, as it was only tested once.
%  \item In the new version (see below), the model was fit to 20 separate batches of data, in the same way (ie for increasing number of lines). Now the spread is the $90\%$ CI in the estimates for those parameters (ie we have 20 means from the 20 runs, this shows how much the estimates vary, for different batches of data).
%  \item Observe: the estimates get closer together, as well as closer to the true value, as the number of lines increases. This gives better evidence for the highlighted sentence at the beginning of this section. As I read it that's kind of the point of this part of the paper.
%\end{itemize}
%
%
...
% USE ANNOYING DATA --- case study has applicable lessons
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{supplement}
%\stitle{Code availability}
%\sdescription{All code is available at \url{https://github.com/cmbaker00/container-line-analysis-public}.}
%\end{supplement}
%
%\begin{supplement}
%\stitle{Data availability}
%\sdescription{The data that support the findings of this simulation study are openly available at \url{https://github.com/cmbaker00/container-line-analysis-public}. The case study data are available from the Australian Department of Agriculture, Fisheries and Forestry. Restrictions apply to the availability of these data, which were used under license for this study.}
%\end{supplement}
...
\bibliography{container-line.bib}       % Bibliography file (usually '*.bib')
