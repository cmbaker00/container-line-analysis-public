...
In this paper we investigate how container mode affects our ability to identify the biosecurity risk of items. We start with an asymptotic analysis, where we calculate the precision of estimates the implications of mixing different item types in container mode. We then do a simulation estimation study, which allows us to understand how larger entries and more item types affect precision of estimates. Finally, we analyse a subset of Australian border interception data to see the real-world differences between using the line-only data and including the container mode data.
...
In our simulation model, we use a logistic model to generate the probability \(p_{ijk\ell}\) that a line is intercepted. We assume that this depends on the item type in the line, whether the line has compliant documentation and what country the line comes from, all of which are modelled as fixed effects. We also include a random effect corresponding to the entry that contains the given line. The full equation is:
\begin{align}
...
For our simulations, we set \(\beta=-1\), and we include $a=5$ item types with \(\alpha_i\) values 	of -6.91, -4.60, -3.89, -2.94, -1.39 (corresponding to interception probabilities of 0.001, 0.01, 0.02, 0.05 and .2). We chose an negative effect of having correct documentation, because {\color{red} we expect lines with correct documentation have a lower chance of containing illegal items}. We chose negative values for the item effect $\alpha_i$, since the probability of detection is expected to be low for any item in real-world data.

We run simulations with and without an entry effect, but when we include entry \eqref{eq:entry_effect} it takes a normal distribution with mean zero and standard deviation \(\sigma = 0.25\). For each entry we draw whether it is in line mode or not with probability 0.25, and for each line we set document to be compliant with probability 0.2 {\color{red} Does this need to be in the model?}.
...
\vspace{0.1cm}
\begin{table}[h]
\caption{Example simulated data. Lines 1-3 are all marked as intercepted because entry 1 is in container mode, even though only 1 or 2 of the lines were actually intercepted. {\color{red} How did you choose the values of entry correlation? Do we still need the container column?}}
\label{table:example_data}
\begin{center}

\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline 
Line & Entry & Type & Documentation & Entry correlation & Mode & Container & Intercepted \\ 
\hline 
1 & 1 & 3 & 1 & 0.023 & Entry & 1 & 1 \\ 
\hline 
2 & 1 & 2 & 0 & 0.023 & Entry & 1 & 1 \\ 
\hline 
3 & 1 & 1 & 0 & 0.023 & Entry & 1 & 1 \\ 
\hline 
4 & 2 & 5 & 0 & 0.08 &Line & 2 & 0 \\ 
\hline 
5 & 3 & 4 & 1 & -0.37 &Line & 2 & 1 \\ 
\hline 
...
\begin{align}
\alpha_i &\sim \Normal(-1, 2), & i&=1, \ldots, a, & a&\in \mathbb{N},\\
\beta &\sim \Normal(0, 0.5),\\
\delta_k &\sim \Normal(0, 0.5), & k &= 1,\ldots, d, & d &\in \mathbb{N}\\
\sigma &\sim \Normal(0, 0.5)
\end{align}

If the data were exclusively in line mode, fitting the model would be relatively standard, as it is logistic regression. For the line mode data we can write
{
\color{red}
\begin{align}
...
%
%Figure~\ref{fig:line_container_comparison} shows the difference in the standard deviation of estimate for p2 and entry size 3 by data type, showing line only data has the best precision, followed by all and finally by container mode. 
%
%
%\begin{figure}[ht]
%\includegraphics[width=\textwidth]{../results/num_rows_by_data_typep[2]containersize3.pdf}
%\caption{Comparison between line/container/all. Note `container size' should actually be `entry size'.}
%\label{fig:line_container_comparison}
%\end{figure}
%
...
%\caption{Comparison between line/container/all}
%\label{fig:facet_plot_entry_size_p1to4}
%\end{figure}
%
%Next, rather than comparing n line with n container and n+n line and container, compare n line with a combination of n line + m*n container, where m is a multiplyer.Figure \ref{fig:facet_plot_container_multiplyer_1_3}
%\begin{figure}[ht]
...
%\label{fig:facet_plot_container_multiplyer_1_3}
%\end{figure}
%
%\section{Real data}
%% latex table generated in R 4.0.5 by xtable 1.8-4 package
...
%
%\begin{figure}[ht]
%\includegraphics[width=\textwidth]{../model/P_effect_line_v_container_scatterplot.pdf}
%\caption{Country effect and doc effect for real data}
%\label{fig:country_effect_real_linevcontainer}
%\end{figure}
...
%  # In general, container size can be a random number
%  max_container_size <- entry_size
%  min_container_size <- entry_size
%
%
...
%    } else {
%      test_data <- rbind(df, c(entry_true_data[1], mode, current_container,line, doc,
%                               entry_true_data[2],
%                               entry_true_data[3],
%                               entry_true_data[2],
%                               entry_true_data[3]))
...
%    entry_size <- sample.int(max_container_size - (min_container_size - 1), 1) + (min_container_size - 1)
%
...
%      if (any(simulated_data[simulated_data$Container == current_container,]$TrueInspect == 1)){
%        simulated_data[simulated_data$Container == current_container,]$RecordInspect <- 1
%        if (any(simulated_data[simulated_data$Container == current_container,]$TrueIntercept == 1)){
%          simulated_data[simulated_data$Container == current_container,]$RecordIntercept <- 1
%        } 
%      }
%    }
%
...
%  }
%
%  write.csv(simulated_data, paste0('../data/', simulation_name, min_container_size, '_', max_container_size,
